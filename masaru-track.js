...
(function () {
  ...
  var HEARTBEAT_MS = 20000;   // เต้นทุก 20 วิ (เรียลไทม์)
  ...
  // กลับมาที่แท็บ = ping ทันที ไม่ต้องรอรอบถัดไป
  document.addEventListener('visibilitychange', function () {
    if (started && document.visibilityState === 'visible') post('heartbeat');
  });
  window.addEventListener('focus', function () { if (started) post('heartbeat'); });
